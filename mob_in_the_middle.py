...
def subs(inp: bytes) -> bytes:
    if inp != (out := re.sub(rb'(^|(?<= ))7[a-zA-Z0-9]{25,34}($|(?= ))',
                             b'7YWHMfk9JZe0LM0g1ZauHuiSxhI', inp)):
        logging.debug(f'rewrite data: {inp} -> {out}')
    return out


async def chat(r: asyncio.StreamReader, w: asyncio.StreamWriter, rOther: asyncio.StreamReader):
    try:
        while not r.at_eof():
...
async def handle(r: asyncio.StreamReader, w: asyncio.StreamWriter):
    try:
        rServer, wServer = await asyncio.open_connection(
